...
from eval_uav import eval_auc

def readfir(video_name):
    with open('./dataset/UAV123/anno/UAV123/'+video_name+'.txt','r') as f:
        l = f.readlines()
        content = l[0]
        contents = []
        content = content.strip().split(',')
        for i in range(len(content)):
            contents.append(float(content[i]))
    f.close()
    return contents
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Test DCFNet on UAV123')
    parser.add_argument('--dataset', metavar='SET', default='UAV123',
                        choices=['vot2018'], help='tune on which dataset')
    parser.add_argument('--model', metavar='PATH', default='param.pth')
    args = parser.parse_args()

    dataset = args.dataset
    base_path = join('./dataset', dataset, 'data_seq/UAV123')   #./dataset/UAV123/data_seq/UAV123
    json_path = join('./dataset', dataset + '.json')    #./dataset/UAV123.json
    ...
    for video_id, video in enumerate(videos):  # run without resetting
        ...
        initial = readfir(video)
        init_rect = np.array(initial).astype(np.float)
        image_name = file_name(join(base_path, video_path_name))
        image_files = [join(base_path, video_path_name, im_f) for im_f in image_name]
