...
from eval_vot import eval_auc

def readfir(video_name):
    with open('./dataset/vot2018/'+video_name+'/groundtruth.txt','r') as f:
        l = f.readlines()
        content = l[0]
        contents = []
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Test DCFNet on VOT')
    parser.add_argument('--dataset', metavar='SET', default='vot2018',
                        choices=['vot2018'], help='tune on which dataset')
    parser.add_argument('--model', metavar='PATH', default='param.pth')
    args = parser.parse_args()

    dataset = args.dataset
    base_path = join('./dataset', dataset)              #./dataset/vot2018
    json_path = join('./dataset', dataset + '.json')    #./dataset/vot2018.json
    ...
    for video_id, video in enumerate(videos):  # run without resetting
        ...
        image_files = [join(base_path, video_path_name, im_f) for im_f in annos[video]['image_files']]
