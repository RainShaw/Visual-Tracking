...
from eval_got10k import eval_auc

def readfir(video_name):
    with open('./dataset/GOT10K/test/'+video_name+'/groundtruth.txt','r') as f:
        l = f.readlines()
        content = l[0]
        contents = []
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Test DCFNet on GOT10K')
    parser.add_argument('--dataset', metavar='SET', default='GOT10K',
                         help='tune on which dataset')
    parser.add_argument('--model', metavar='PATH', default='param.pth')
    args = parser.parse_args()

    dataset = args.dataset
    base_path = join('./dataset', dataset, 'test')   #./dataset/GOT10K/test
    json_path = join('./dataset', dataset + '.json')    #./dataset/GOT10K.json
